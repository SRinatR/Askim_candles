...
export default function AddressesPage() {
  ...
  function onSubmit(data: AddressFormValues) {
    if (editingAddress) {
      ...
      // Add new address
      const newAddress = { ...data, id: `addr${Date.now()}` }; // Simple ID generation
  ...
  if (isFormOpen) {
    return (
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="text-2xl">{editingAddress ? 'Edit Address' : 'Add New Address'}</CardTitle>
        </CardHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <CardContent className="space-y-4">
              <FormField name="street" control={form.control} render={({ field }) => ( <FormItem><FormLabel>Street Address</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField name="state" control={form.control} render={({ field }) => ( <FormItem><FormLabel>State/Province</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField name="zipCode" control={form.control} render={({ field }) => ( <FormItem><FormLabel>ZIP/Postal Code</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
              ...
              <CardContent className="p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                  <div>
                    ...
                    {address.isDefault && <span className="text-xs font-medium text-primary mt-1 inline-flex items-center"><Home className="h-3 w-3 mr-1" /> Default</span>}
                  </div>
                  <div className="flex space-x-2 mt-3 sm:mt-0 shrink-0">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => handleEdit(address)} aria-label="Edit address">
                      <Edit className="h-4 w-4" />
                    </Button>
                    <AlertDialog>
                        <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive hover:bg-destructive/10" aria-label="Delete address">
